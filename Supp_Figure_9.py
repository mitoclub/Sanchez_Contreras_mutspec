...
from GlobalVars_ import tissue_type, tissue_type_long, old_nmn_id, color_cycle, \
                        tissue_type_abbrev
...
if __name__ == "__main__":
    ...
    for index, tissue in enumerate(tissue_type[:-1]):
        ...
        if not os.path.isfile("data/imported_data/Supplemental_Figure_10_" + tissue_type_abbrev[index] + "_depth_data.csv"):
            if not os.path.isdir("data/stats/"):
                os.mkdir("data/stats/")
            
            depth_df = calc_depth_plot_data(old_nmn_id, tissue,
                                            "data/imported_data/Supplemental_Figure_10_" + tissue_type_abbrev[index] + "_depth_data.csv")
        else:    
            depth_df = calc_depth_plot_data(old_nmn_id, tissue)
        ...
        axs[index].set_title(tissue_type_long[index], 
                             fontsize=12
                             )
        axs[index].tick_params(labelsize=10)
    
    axs[7].set_xlabel('Genome Position', size=12)
    fig.suptitle("Nicotinamide Mononucleotide Mouse Depth", y=0.93, fontsize=12)
    ...
    fig.savefig('figures/Supp_Figure_9.png', dpi=600, facecolor='white', 
                bbox_inches='tight')    
    fig.savefig('figures/Supp_Figure_9.pdf', dpi=600, facecolor='white', 
                bbox_inches='tight')    
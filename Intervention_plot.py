...
def setup_figure():
    ...
    ax['A'].set_ylim(0, 9e-6)
    ax['B'].set_ylim(0, 1e-5)
    ax['C'].set_ylim(0, 9e-6)
    ax['D'].set_ylim(0, 8e-6)
    ax['E'].set_ylim(0, 5e-6)
...
if __name__ == '__main__':
    ...
    data = import_data()
    fig, ax = setup_figure()
    subplot_id = {0: 'A', 1: 'B', 2: 'C', 3: 'D', 4: 'E'}
    intervention_color = color_cycle[1:6]

    for i, tissue in enumerate(tissue_type[1:6]):

        j = subplot_id[i]

        plot = sns.barplot(x="Class", y="Frequency", hue="Treatment",
                           data=data.query("Tissue==@tissue"), order=mut_type,
                           ci='sd', edgecolor='black', lw=1.5, errwidth=1.5,
        ...
        ax[j].set_title(tissue_type_long[1:6][i], fontsize=16, y=0.85)
    ...
    fig.savefig("figures/Remaining_intervention_plots.png", facecolor='white', dpi=600)
